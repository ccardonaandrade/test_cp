...
Which products lead the pack in customer demand? Here's a snapshot of the top performers by quantity ordered:
...
Unfulfilled orders can signal bottlenecks. Here are the top 3 clients most affected, along with the overall total:
...
order_shares_daily <- data |>
  group_by(entry_date, region_clean) |>
  summarise(region_quantity = sum(order_quantity), .groups = "drop") |>
  group_by(entry_date) |>
  mutate(
    total_daily_quantity = sum(region_quantity),
    share = region_quantity / total_daily_quantity
  )



ggplot(order_shares_daily, aes(x = entry_date, y = share, fill = region_clean)) +
  ...
  labs(
    title = "Daily Share of Order Quantities by Region",
    x = "Date",
...
The plot below reveals whether they choose to reorder the same product or not.

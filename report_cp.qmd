...
p6 <- ggplot() +
  geom_line(data = base_data, aes(x = entry_date, y = avg_ins_inventory), color = "steelblue") +
  geom_line(data = highlight_data, aes(x = entry_date, y = avg_ins_inventory), color = "darkred") +
  scale_x_date(
    limits = as.Date(c("2022-06-01", "2023-05-31")),
    date_labels = "%b"  # Format as abbreviated month name
  ) +
  ...
  labs(
    x = "Date", y = "", 
    subtitle = "Average Non-Delivered Orders")
